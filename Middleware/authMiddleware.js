...
const tokenExpiryTime = "1h"; // 1 minute
const refreshExpiryTime = "7d"; // 7 days
...
const createToken = (nhanVien) => {
    return jwt.sign(
        { id: nhanVien._id, vaiTro: nhanVien.vaiTro },
        secretKey,
...
const authenticateFirebaseToken = async (req, res, next) => {
    ...
    if (!authHeader || !authHeader.startsWith("Bearer ")) {
        return res.status(401).json({ error: "Token không được cung cấp hoặc không hợp lệ" });
    }
    ...
    try {
        ...
        let nhanVien = await NhanVien.findOne({ soDienThoai: parsePhoneNumber(phoneNumber) });
        if (!nhanVien) {
